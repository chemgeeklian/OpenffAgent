...
from openff.toolkit import ForceField as OpenFFForceField, Topology as OpenFFTopology
...
class Simulator:
    """
    Class for performing OpenMM simulations on AMBER FF inputs or OpenFF Interchange.
    ...
    Arguments:
        ...
        out_path (Optional[Path]): Defaults to None. Optional output path which is where we
            ...
            `self.path`.
        ff (str): Switch for whether to utilize AMBER, CHARMM, or OpenFF. Options: 'amber',
            'charmm', 'openff'.
        topology_json (Optional[str]): Path to OpenFF topology JSON file. Required if ff='openff'.
        forcefield_file (Optional[str]): Path to OpenFF force field file (.offxml). Required if ff='openff'.
    ...
    def __init__(self,
                 ...
                 forcefield_file: Optional[str]=None,
    ...
    def load_system(self) -> System:
        ...
        Raises:
            AttributeError: If a non-valid choice of forcefield is made (not amber, charmm, or openff).
        ...
        if self.ff == 'amber':
            ...
            system = self.load_openff_interchange()
        else:
            raise AttributeError(f'self.ff must be a valid MD forcefield [amber, charmm, openff]!')
    ...
    def load_openff_interchange(self) -> System:
        """Builds an OpenMM system using OpenFF Interchange from topology JSON and force field.

        Returns:
            (System): OpenMM system
        """
        if not hasattr(self, 'interchange'):
            # Load OpenFF topology from JSON
            with open(str(self.topology_json), "r") as f:
                json_str = f.read()

            openff_topology = OpenFFTopology.from_json(json_str)

            # Load OpenFF force field
            openff_forcefield = OpenFFForceField(str(self.forcefield_file))

            # Create Interchange object
            self.interchange = openff_forcefield.create_interchange(openff_topology)

            # Get OpenMM topology and positions from Interchange
            self.topology = self.interchange.to_openmm_topology()
            self.coordinate = type('obj', (object,), {
                'positions': self.interchange.positions.to_openmm()
            })()

        # Create OpenMM system from Interchange
        system = self.interchange.to_openmm()
    ...
    def setup_sim(self,
        ...
        # Handle different topology types (AMBER/CHARMM have .topology attribute, OpenFF is already a topology)
        if hasattr(self.topology, 'topology'):
    ...
    def get_restraint_indices(self,
        ...
        """
        if self.ff == 'openff':
            # For OpenFF, get backbone atoms directly from OpenMM topology
            indices = []
            topology = self.topology if not hasattr(self.topology, 'topology') else self.topology.topology
            for atom in topology.atoms():
                if atom.residue.name in ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU',
                                         'GLY', 'HIS', 'ILE', 'LEU', 'LYS', 'MET', 'PHE',
                                         'PRO', 'SER', 'THR', 'TRP', 'TYR', 'VAL', 'KPI']:
                    if atom.name in ['CA', 'C', 'N', 'O']:
                        indices.append(atom.index)
            return indices
        else:
            # For AMBER/CHARMM, use MDAnalysis
